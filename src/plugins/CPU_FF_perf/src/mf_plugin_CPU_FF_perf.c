...
init_mf_plugin_CPU_FF_perf(PluginManager *pm)
{
     ...
     */
    PluginManager_register_hook(pm, "mf_plugin_CPU_FF_perf", mf_plugin_CPU_FF_perf_hook);
    is_initialized = 1;
    return ret;
}
...
char*
mf_plugin_CPU_FF_perf_hook()
{
    if (is_initialized) {
        /*
         * sampling 
         */
        mf_CPU_FF_perf_sample(monitoring_data);
        ...
        mf_CPU_FF_perf_to_json(monitoring_data, conf_data->keys, conf_data->size, json);
